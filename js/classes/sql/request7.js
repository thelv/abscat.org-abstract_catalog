...
//{
	...
	var selectEach=function(selectParams, separate, keepMain)
	{
		return pathsForEach(selectParams[2], function(path)
		{
			selectParams[2]=[path];
			return select.apply(null, selectParams);
		});
	}
		...
		switch(select_.type)
		{
			case ',':
				...
				else
				{
					...
					var resAFailB=selectEach(['data', select_.b, [path], branch, null, notResult], true, true);									
					return {
						...
						if(path._mainSelect)
						{
							...
							{			
								...
								var childSelect=
								{
									...
									notSelect: select_.notSelect, notSelectUncond: ! select_.alias, //(select_.a.type!='|' && select_.a.type!='^' && select_.a.type!='u'), 
							...
							var a=select('data', childSelect, [path], childBranch, null, notResult);
							return a;
						}
						else
						{
				...
				{	
					...
					for(var i in paths_)
					{
						...
						for(var i in path.aliases)
						{
							...
							if(alias.cont==select_.connection && Branch.toStringAlias(select_Branch)==Branch.toStringAlias(alias.baseBranch))
							{
