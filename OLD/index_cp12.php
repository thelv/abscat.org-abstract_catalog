<html>
	<head>
		...
		<style id="style_object_filter">
			.table [data_type]{display:-none}
			.table [data_type = song]{font-size:40px !important;display:block}
			.table [data_text]{display:none}
			.table [data_text~=mad]{display:block}
		</style>
	</head>
	<body>
		<div id="head"><div>
		...
		<div id="body">
			<div id="page_my_catalog" class="page">
				...
				<div class="body_body">
					<div class="object_filter">
						Object filter: type~ <input id=object_filter_type type=text>, text~ <input id=object_filter_text type=text>
					</div>
			...
			<script>		
				...
				{
					console.log('call:'+objectId+' '+connectionId+' '+connectionStraightOrOpposite);
					var this_=this;
				...
				var table=function()
				{
					...
					{					
						...
						');
						typeNode.attr('data_type', type.type);
						...
						for(var i in type.objects)
						{
							...
							objectNode.text(object.text).css('color', object.color).attr('title', object.type).attr('text', object.text);
							(function(object)
							{
				...
				{
					var type=$('#object_filter_type').val();
					var text=$('#object_filter_text').val();
					$('#style_object_filter').html(
					'\
						.table [data_type]{display:none}\
						.table [data_type~='+type+']{display:block}\
						.table [data_text]{display:none}\
						.table [data_text~='+text+']{display:block}\
					');
				}
				...
				var page=function(page_, params)
				{
					if(page_=='my_catalog')
					{
						...
						$('#page_my_catalog .object_table').append(table());
