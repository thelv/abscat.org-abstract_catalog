<html>
			...
			#body{font-size:14px;overflow:hidden;margin:6px 10px;margin-top:37px;}							
				...
				.body_body{margin-top:6px}
					...
					#page_my_catalog{}
						...
						.table{vertical-align:top;margin-top:11px}
							.table tr{vertical-align:top}
							.table td{vertical-align:top;}
							.table_type_head{font-weight-:bold;font-style-:italic;margin-bottom:7px;padding-bottom:8px;border-bottom:1px solid #ccc;padding-right:29px;color:#000}
							.table_type_objects{padding-right:29px}
								.table_type_object{margin-bottom:5px;cursor:pointer}
									.table_type_object:hover{text-decoration:underline}
						
					#page_my_catalog_object{}
						#trees{}														
							.tree{margin-top:5px}
	...
	<body>
		...
		<div id="body">
			<div id="page_my_catalog" class="page">
				<div class="body_head">
					My catalog
				</div>
				<div class="body_menu">
			...
			<div id="page_my_catalog_object" class="page">
				<div class="body_head">
					My catalog - Object
				</div>
				<div class="body_menu">
			...
			<script>		
				...
				var Branch=function(objectId, connectionId, connectionStraightOrOpposite, parent)
				{
					...
					.
						click(function()
						{
							if(! objectId)
							{
								page('my_catalog_object', {objectId: this_.objectId});
								return;
							}
							...
							[
								...
								,
								{
									...
									action: function()
									{
										windows.objectConnectWithObjectFromBuffer.open(false, function(data_)
										{
				...
				var table=function(filterType, filterText)
				{
					var types=[];
					...
					types=types.sort(function(a, b){return a.type>b.type;});
					...
					{					
						...
						');
						typeNode.find('.table_type_head').text(type.type);
						var typeObjectsNode=typeNode.find('.table_type_objects');
						
						for(var i in type.objects)
						{
							var object=type.objects[i];
							var objectNode=$('<div class=table_type_object></div>');
							objectNode.text(object.text).css('color', object.color).attr('title', object.type);
							(function(object)
							{
								objectNode.click(function()
								{
										page('my_catalog_object', {objectId: object.id});
								});
							})(object);
				...
				windows=
				{
					...
					{
						this.open=function(data_, callback)
						{												
							if(! buffer.element)
							{
				...
				var page=function(page_, params)
				{
					if(page_=='my_catalog')
					{
						$('.page').hide();
						$('#page_my_catalog .object_table').html('');
						
						var type=$('#page_my_catalog .object_filter_type').val();
						var text=$('#page_my_catalog .object_filter_text').val();
						$('#page_my_catalog .object_table').append(table(type, text));
						$('#page_my_catalog').show();
					}
					else if(page_=='my_catalog_object')
					{
						$('.page').hide();
						$('#page_my_catalog_object .body_body').html('');
						$('#page_my_catalog_object .body_body').append((new Branch(params.objectId)).node);	
						$('#page_my_catalog_object').show();
					}
				}
				...
				$(function()
				{
					...
					//bind page "my catalog"
					
						$('#page_my_catalog .object_filter form').submit(function()
						{
							page('my_catalog');
							return false;
						}).on('reset', function()
						{
							setTimeout(function(){page('my_catalog');});;
						});
						
						$('#page_my_catalog .body_menu ._object_new').click(function()
						{
							windows.objectNew.open(null, function(data_)
							{
								...
								page('my_catalog_object', {objectId: objectId});
							}); 
						});
							
					//bind page "my catalog object"
						
						$('#page_my_catalog_object .body_menu > ._back').click(function(){page('my_catalog');});						
