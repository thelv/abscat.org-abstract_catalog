		...
		<style>
			*{padding:0;margin:0;}
			...
			#body{font-size:14px;overflow:hidden;margin:6px 10px;margin-top:37px;}							
				...
				.catalog_createobject_link{display-:none;padding-top:7px}
			...
			#objects_filter{margin-top:11px}
			#objects_filter input{width:100px}
			
			#list{vertical-align:top;margin-top:11px}
				#list tr{vertical-align:top}
				#list td{vertical-align:top;}
				.list_head{font-weight-:bold;font-style-:italic;margin-bottom:7px;padding-bottom:8px;border-bottom:1px solid #ccc;padding-right:29px;color:#000}
				.list_body{padding-right:29px}
					.list_body > div{margin-bottom:5px}
				#list_type_song{color:#22f}
				#list_type_genre{color:#e44}
				#list_type_source{color:#3a6}
		</style>
	</head>
	<body>
		<div id="head"><div>
		...
		<div id="body">							
			...
			<div id=objects_filter>
				Objects filter: type~ <input type=text>, text~ <input type=text>
			</div>
			
			<table id=list cellpadding=0 cellspacing=0>
				<tr>
					<td id=list_type_song>
						<div class=list_head>
							song
						</div>
						<div class=list_body>
							<div>
								Madonna - Sorry for Love
							</div>
							<div>
								The Lift - Conditioner Report
							</div>
						</div>
					</td>
					<td id=list_type_genre>
						<div class=list_head>
							genre
						</div>
						<div class=list_body>
		...
		<script>		
			...
			windows=
			{
				...
				objectRemove: new Window('window_object_remove').extend(function()
				{
					this.open=function(object, callback)
					{															
						this.node.find('._form ._head').text('Remove object "'+object.text+'"?');						
				...
				connectionRemove: new Window('window_connection_remove').extend(function()
				{
					this.open=function(connection, callback)
					{															
						this.node.find('._form ._head').text('Remove connection "'+connection.text+'"?');						
